...
Finally, it should call append() which is similar to update().
...
# I shd demonstrate .append():
values_l = [
  [1.1, 2.9, 3.5],
  [1.3, 2.8, 3.9]
]
body_d   = {'values': values_l}
range_s  = 'Sheet1!A8'

response_ofappend = service.spreadsheets().values().append(
    spreadsheetId=spreadsheet_id, range=range_s,
    valueInputOption='USER_ENTERED', body=body_d).execute()

print("response_ofappend.get('updates'):")
print( response_ofappend.get('updates')  )
